...
def build_tab(engine: Any) -> TabUI:
    ...
    with gr.Row(equal_height=False):
        with gr.Column(scale=5, elem_classes=["jc-compact"]):
            ...
            with gr.Row():
                caption_btn = gr.Button("Caption Image", elem_classes=["btn-pre-caption"])
        ...
        with gr.Column(scale=4, elem_classes=["jc-compact"]):
            with gr.Accordion("Generation", open=True):
                ...
                with gr.Row():
                    components["do_sample"] = gr.Checkbox(label="Do Sample", value=DEFAULTS["do_sample"])
                    components["top_k"] = gr.Slider(1, 100, value=DEFAULTS["top_k"], step=1, label="Top K")
                    components["temperature"] = gr.Slider(0.1, 2.0, value=DEFAULTS["temperature"], step=0.1, label="Temperature")
                with gr.Row():
                    components["padding"] = gr.Checkbox(label="Padding", value=DEFAULTS["padding"])
                    components["truncation"] = gr.Checkbox(label="Truncation", value=DEFAULTS["truncation"])
                    components["add_special_tokens"] = gr.Checkbox(label="Add Special Tokens", value=DEFAULTS["add_special_tokens"])
                components["custom_prompt"] = gr.Textbox(label="Custom System Prompt", lines=3, value=DEFAULTS["custom_prompt"])
    ...
    with gr.Accordion("Universal Folder Batch", open=True):
        ...
        with gr.Row():
            batch_btn = gr.Button("Start Pre-Alpha Folder Batch", elem_classes=["btn-pre-batch"])
            stop_btn = gr.Button("Cancel Batch", elem_classes=["btn-pre-stop"])
